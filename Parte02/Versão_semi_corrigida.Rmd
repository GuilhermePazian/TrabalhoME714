...
Em 1990, realizou-se um estudo para compreender a influência de certos fatores na quantidade de infecções auditivas de recrutas americanos. Este estudo considerou algumas variáveis como: faixa etária (15-19 anos, 20-24 anos, 25-29 anos), sexo (masculino ou feminino), local de nado (piscina ou praia), frequência com a qual o recruta praticava natação (ocasional ou frequente) e o número de infecções relatadas pelo recruta. 

Considerando estas variáveis, o objetivo desta análise é reproduzir esse estudo à ótica do modelo log-linear do Possion e avaliar, com base nos resultados obtidos, o impacto de cada um desses fatores para o número de infecções auditivas nos recrutas americanos. Abaixo, segue a análise descritiva
...
- A quantidade de indivíduos que têm pelo menos uma infecção, para o grupo cujo local costumeiro de natação é a praia, é menor do que no grupo que não costuma nadar na praia;
...
Note, nas figuras 4 e 5, que o comportamento dos perfis para o hábito de nadar é muito semelhante para o sexo e para o local usual de nado. Consequentemente, podemos visualizar que, na ausência de interação entre os fatores hábito de nadar, sexo e local usual de nado (sem interação de segunda ordem). Assim, a interação de terceira ordem, isto é, a combinação de todos os fatores, pode ser descartada.
...
& Piscina & 243 \\ 
   \hline
\end{tabular}
\captionof{table}{Tabela com o cômputo de infecções para os diferentes níveis do lugar onde se costuma nadar.}
\end{table}
...
& 25-29 &  83 \\ 
   \hline
\end{tabular}
\captionof{table}{Tabela com a contagem de infecções para os diferentes níveis de faixa etária.}
\end{table}
...
& Masculino & 25-29 &  53 \\ 
   \hline
\end{tabular}
\captionof{table}{Tabela com as quantidades do número de infecções para as combinações dos sexos às diferentes faixas etárias.}
\end{table}
...
fig4 <- "Gráfico de perfil que representa o número de infecções para os indivíduos que tem um determinado hábito de
nadar, para os diferentes sexos. Observa-se que as retas não são paralelas. Com isso, pode-se pensar que existe interação
entre o sexo e o hábito de nadar."

fig5 <- "Gráfico de perfil que representa o número de infecções para os indivíduos que têm determinado hábito de nadar e o local no qual costumam nadar. O comportamento é muito similar em relação à Figura 4 e cogita-se uma possível interação entre o local no qual costuma nadar e o hábito de nadar."

fig6 <- "Gráfico de perfil que representa o número de infecções para os sexos, para indivíduos que tem costume de nadar em diferentes lugares. Nota-se que é plausível a interação entre sexo e o lugar no qual se costuma nadar"
```
...
Modela-se o logarítimo da de $\mu_{i}$ para cada indivíduo e para isso, considera-se as covariáveis em estudo. Observe a seguinte estrutura do possível modelo completo:
...
O valor do desvio observado e do AIC para o modelo completo foi de `r round(fit_completo$deviance, 2)`, `r round(fit_completo$aic,2)`, respectivamente.
...
- Retirada a interação de terceira ordem (agora, tem-se o modelo de segunda ordem), será reavaliado a significância dos efeitos conjuntamente nos modelos por meio do teste $C \beta = M$;
