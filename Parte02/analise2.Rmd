...
- A quantidade de indivíduos que têm pelo menos uma infecção, para o grupo cujo hábito de nadar é frequente, é menor do que para o grupo que nada ocasionalmente;

- A quantidade de indivíduos que possuem pelo menos uma infecção, para o grupo cujo local onde costuma nadar é a praia, é menor do que para o grupo que não costuma nadar na praia;

- A quantidade de indivíduos que têm pelo menos uma infecção, para a faixa etária 15-19, é maior do que para os grupos das demais faixas; e o número de indivíduos que tem pelo menos uma infecção na faixa etária 20-24 é maior do que a faixa etária 25-29;

- A quantidade de indivíduos que têm pelo menos uma infecção, para o sexo masculino, é maior do que para o para o sexo feminino;
...
Modela-se o logarítimo da de $\mu_{i}$ para cada indivíduo e para isso, considera-se as covariáveis em estudo. A estrutura abaixo ilustra um possível modelo (neste caso, o modelo é completo com todas as interações de terceira ordem):
...
Primariamente o modelo completo será avaliado, isto é, aquele naqual a interação de terceira ordem está presente na especificação das variáveis. Abaixo tem-se as análises de resíduos pertinentes ao modelo proposto inicialmente.
...
```{r modelo_poisson_cbetam, echo=FALSE}
...
```{r diagnostico_modelo_comlpleto, fig.cap= fig7, fig.pos="H", fig.asp=0.75,echo=FALSE}
...
Observando os resultados do modelo completo , avalia-se:

- A não significância de interação de terceira ordem, não significância de algumas interações de segunda ordem (nota-se que uma destas não significâncias, foi observada para a interação de hábito de nadar, local onde costuma nadar e sexo. Característica já observada na análise descritiva como passível de ausência de interação). A princípio, será avaliado o modelo sem a interação de terceira ordem (uma vez que interações de segunda ordem são não significativas);

- Não obstante a descrição anterior, retirar-se-á a interação de terceira ordem (agora, tem-se o modelo de segunda ordem) e será reavaliado a significância dos efeitos conjuntamente nos modelos por meio do teste $C \beta = M$;

Devido o contexto da disciplina, mesmo o modelo não apresentando bom ajuste, continuaremos com a análise, portanto avaliar-se-á modelos em que os fatores principais e as interações de segunda ordem serão avaliados, e consequentemente por meio do referido teste se escolherá os efeitos que ficarão no modelo final. Pela análise descritiva, notou-se possivelmente a não interação para alguns fatores de segunda ordem. 

Depois de vários testes feitos, a seleção de variáveis para o modelo final encontrado levou em consideração algumas fatores principais como hábito de nadar, local onde costuma nadar e faixa etária e suas respectivas interações de terceira ordem: interações entre hábito, faixa etária,  e local onde costuma nadar.Como representado abaixo:

$$
ln(\mu_i) = \mu + \alpha O_i + \gamma P_i + \beta_1 (20-24)_i + \beta_2 (25-29)_i + 
$$
$$
(\alpha  \gamma) P_i O_i + (\alpha \beta_1) (20-24)_i O_i + (\alpha \beta_2) (25-29)_iO_i + (\beta_1 \gamma) (20-24)_iP_i + (\beta_2 \gamma) (25-29)_iP_i
$$

$O_i$ = 1 se o i-ésimo indivíduo nada ocasionalmente e $O_i$ = 0 se nada frequente.  
$P_i$ = 1 se o i-ésimo indivíduo costuma nadar na piscina e $P_i$ = 0 costuma nadar na praia.  
$(20-24)_i$ = 1 se o i-ésimo indivíduo pertence a faixa etária 20-24 e $(20-24)_i$ =  0 caso não pertença.  
$(25-29)_i$ = 1 se o i-ésimo indivíduo pertence a faixa etária 25-29 e $(25-29)_i$ = 0 caso não pertença.
...
$e^\mu$ - Recruta com idade entre 15 e 19 anos que nada com frequência na praia.  

$e^{\mu + \beta_1}$ - Recruta com idade entre 20 e 25 anos que nada com frequência na praia.  

$e^{\mu + \beta_2}$ - Recruta com idade entre 25 e 29 anos que nada com frequência na praia.  

$e^{\mu + \alpha}$ - Recruta com idade entre 15 e 19 anos que nada ocasionalmente na praia.

$e^{\mu + \alpha + \beta_1 + (\alpha \beta_1)}$ - Recruta com idade entre 20 e 25 anos que nada ocasionalmente na praia.  

$e^{\mu + \alpha + \beta_2 + (\alpha \beta_2)}$ - Recruta com idade entre 25 e 29 anos que nada ocasionalmente na praia.  

$e^{\mu + \gamma}$ - Recruta com idade entre 15 e 19 anos que nada com frequência na piscina.  

$e^{\mu + \gamma + \beta_1 + (\beta_1 \gamma)}$ - Recruta com idade entre 20 e 25 anos que nada com frequência na piscina.    

$e^{\mu + \gamma + \beta_2 + (\beta_2 \gamma)}$ - Recruta com idade entre 25 e 29 anos que nada com frequência na piscina.    

$e^{\mu + \gamma + \alpha + (\alpha \gamma)}$ - Recruta com idade entre 15 e 19 anos que nada com frequência na piscina.  

$e^{\mu + \gamma + \beta_1 + \alpha + (\alpha \gamma) + (\alpha \beta_1) + (\beta_1 \gamma)}$ - Recruta com idade entre 20 e 25 anos que nada com frequência na piscina.    

$e^{\mu + \gamma + \beta_2 + \alpha + (\alpha \gamma) + (\alpha \beta_2) + (\beta_2 \gamma)}$ - Recruta com idade entre 25 e 29 anos que nada com frequência na piscina.  

O valor do desvio observado e do AIC para o modelo final reduzido foi de `r round(fit.model_p$deviance, 2)`, `r round(fit.model_p$aic,2)`. Nota-se que, embora estes valores sejam maiores em relação ao modelo completo, características de qualidade de ajuste observado pelos gráficos do modelo final se apresentam razoavelmente melhor. É importante lembrar-se de que considerar o valor do desvio, faz com que características como o preditor e a função de ligação sejam desconsideradas, isto é, o desvio não leva em consideração tais características. 
...
& Estimativa & Erro Padrão & Estatística Z & P-valor \\ 
  \hline
Intercepto($\mu$) & 0,1356 & 0,1502 & 0,9029 & 0,3666 \\ 
  Hábito (Ocasional) ($\alpha$) & 0,2421 & 0,1886 & 1,2838 & 0,1992 \\ 
  Local  (Piscina) ($\gamma$)  & 0,1534 & 0,1904 & 0,8055 & 0,4205 \\ 
  Faixa etária 20-24 ($\beta_1$)& -0,8041 & 0,2879 & -2,7931 & 0,0052 \\ 
  Faixa etária 25-29 ($\beta_2$)& -0,7509 & 0,2762 & -2,7185 & 0,0066 \\ 
  Hábito (Ocasional):Faixa etária 20-24 ($\alpha\beta_1$)& 0,0776 & 0,2612 & 0,2971 & 0,7664 \\ 
  Hábito (Ocasional):Faixa etária 25-29 ($\alpha\beta_2$)& 0,7112 & 0,2930 & 2,4272 & 0,0152 \\ 
  Hábito (Ocasional):Local (Piscina) ($\alpha\gamma$) & 0,3555 & 0,2217 & 1,6038 & 0,1088 \\ 
  Local (Piscina):Faixa etária 20-24 ($\gamma\beta_1$) & 0,5696 & 0,2882 & 1,9763 & 0,0481 \\ 
  Local (Piscina):Faixa etária 25-29 ($\gamma\beta_2$) & 0,1346 & 0,2620 & 0,5137 & 0,6075 \\ 
   \hline
\end{tabular}
...
Observamos pelo modelo ajustado que as covariáveis "hábito de nadar", "local onde costuma nadar" e "faixa etária" são as mais impactantes no número de infecção de ouvido. Mesmo quando o efeito principal destas covariáveis não são significativas (a um nível de significância de 5%), como é o caso das variáveis "hábito de nadar" e "local onde costuma nadar", alguma interação com algum dos fatores da covariável "faixa etária" o é, portanto, mesmo não tendo impacto significativo do efeito principal as covariáveis têm impacto no número de infecções dos ouvidos por ter interação estatisticamente significativa com a covariável a qual tem efeito principal significativo.

```{r graficos_de_diagnostico, fig.cap=fig9, fig.asp=.75, fig.pos="H", echo=FALSE}
...
O gráfico de envelopes apresentado acima têm um comportamento de deslocamento anti-horário em relação ao gráfico de envelopes, comportamento o qual é um bom indicativo de presença de super dispersão sob o modelo, portanto  pode-se pensar que existe uma superdispersão dos dados que não é captada pelo modelo proposto e ajustado. Logo, uma possível alternativa seria utilizar um modelo que contemple a superdispersão. Neste caso, será avaliado o ajuste com o modelo
binomial negativo.
...
```{r diagnostico_BN, fig.cap = fig11, fig.pos="H", fig.asp=0.75,echo=FALSE}
