...
df_111 = data.frame(df_11,totais)

rownames(df_111)=c("Allbond","Optibond","Scotchbond")
colnames(df_111)=c("0","1","2","3","4")
...
```{r,echo=FALSE, warning = FALSE,eval=TRUE,message = FALSE, error = FALSE}
#análise descritiva
estimado_theta_1 = estima.theta(df_11,2,0.05)
plot.graf.prop(estimado_theta_1,1,0.5)
```

```{r modelo1_exercicio1,echo=FALSE, warning = FALSE,eval=TRUE,message = FALSE, error = FALSE}
...
Objetivos: Descrição do(s) modelo(s), análise(s) de resíduo(s), comparações de interesse, gráficos e
...
O Modelo probabílistico gerador da Tabela é o Produto de Multinomiais pois os totais marginais relacionados ao material de Selante em questão e é dado pela seguinte equação:

$N_{i}=(N_{i1},N_{i2},\dots,N_{i(s-1))})^{'}\sim multinomial_{s}(n_{i.},\mathbf{\theta}_{i})$

$\mathbf{{\theta_{i}}}=(\theta_{i1},\theta_{i2},\dots,\theta_{i(s-1))})^{'}$ ((s-1) parâmetros) e $\theta_{ij} \in  (0,1)$
